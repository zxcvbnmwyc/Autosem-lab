...
手动点选和框选不需要任何外部模型。只有点击“启动分割 Agent”或“只查看 Qwen 候选”时，当前图片和文字描述才会发送给 `.env` 配置的阿里云百炼 Qwen 视觉模型。Agent 不会让 Qwen 直接输出轮廓：它只根据候选数量、状态和用户确认来选择下一步，最终 mask 仍由 SAM2 生成。
