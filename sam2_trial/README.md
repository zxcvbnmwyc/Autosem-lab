...
AutoSEM 是一个本地优先的 AI 选区与局部编辑网站：用户上传一张图片并描述目标，分割 Agent 会决定是直接采用 Qwen（可选）的候选框、请用户选择多个候选，还是请求点选／框选等手动提示；SAM2 最终生成像素级选区。选区完成后，可补画、擦除、收缩／扩展、羽化，并在原图尺寸下导出透明抠图、纯色背景、背景虚化或局部调色版本。

它不是让大模型直接“画边界”：

```text
文字描述 ──可选──> Qwen 视觉定位 ──> Agent 决策 ──> 候选框 / 追问 / 手动提示
                                                          │
...
- `/`：产品首页，解释工作流和数据边界。
- `/workspace`：分割 Agent、Qwen 自动定位、手动点/框、后台任务、质量复核、选区微调与局部编辑。
- `/guide`：使用指南。
...
手动点选和框选不需要任何外部模型。只有点击“启动分割 Agent”或“只查看 Qwen 候选”时，当前图片和文字描述才会发送给 `.env` 配置的阿里云百炼 Qwen 视觉模型。Agent 不会让 Qwen 直接输出轮廓：它只根据候选数量、状态和用户确认来选择下一步，最终 mask 仍由 SAM2 生成。启用内部锚点后，Qwen 在能可靠指出目标内部位置时会额外提供一个包含点，帮助 SAM2 区分框内相邻物体；无法可靠给点时会自动退回为仅使用候选框。
...
测试覆盖轮廓导出、Qwen 响应验证、候选框约束、Agent 的候选选择／手动回退／服务端候选框约束／质量复核，以及后台 job 的提交、轮询、持久化和结果文件访问。
