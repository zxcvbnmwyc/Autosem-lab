...
AutoSEM 是一个本地优先的 AI 选区与局部编辑网站：用户上传图片后，只需用自然语言说出要处理的主体，也可以顺便描述效果。Qwen 会从任意自然表述中识别主体，并把其余意图改写为经过白名单校验的本地编辑计划；SAM2 负责生成像素级选区。只输入主体时流程停在可继续调整的选区，输入受支持的效果时再生成合成预览。手动分割 Agent、点／框提示和选区画笔仍完整保留。选区完成后，可补画、擦除、收缩／扩展、羽化，并导出透明抠图、纯色、虚化或自定义图片背景、主体与背景局部调色、描边、阴影及按主体裁切的高清 PNG。编辑参数和选区笔刷均支持撤销与重做。
...
手动模式：文字描述 ──可选──> Qwen 视觉定位 ──> Agent 决策 ──> 候选框 / 追问 / 手动提示
                                                          │
点 / 框 ─────────────────────────────────────────────────┼──> SAM2 Tiny ──> 质量复核 / 最多一次重试 ──> 可编辑 Mask
                                                                                          │
...
独立的 Qwen 定位阶段只给 SAM2 提供空间提示；Qwen 不能输出任意代码、路径、URL 或 mask 像素，也不能直接生成轮廓。SAM2 生成选区后，只有存在可见编辑效果时才进入方案检查和合成；没有额外效果时直接返回可编辑选区。定位出现多个候选或置信度不足时，会让用户选择对象，不会静默挑一个继续。选区出现明显的空白、范围异常或位置偏离时，系统会在拥有新的可靠内部点时自动重试一次；没有新提示信息、队列繁忙或重试仍不可靠时会保留较好的结果供手动微调，不会无限重跑。当前一键能力覆盖单主体抠图、透明/纯色/模糊背景、边缘优化、主体与背景局部调色、主体柔化或锐化、透明度、描边、阴影，以及围绕主体的 free、1:1、4:5、16:9 裁切；自定义图片背景属于选区完成后的手动编辑能力。删除后补图、添加或替换物体、扩图等生成式部分目前不会被伪装成已完成：若主体明确，系统会跳过无法执行的部分、显示说明，并继续生成该主体的选区或执行其余独立且受支持的操作；若没有主体，才会要求补充。启用内部锚点后，Qwen 在能可靠指出目标内部位置时会额外提供一个包含点，帮助 SAM2 区分框内相邻物体；无法可靠给点时会自动退回为仅使用候选框。
...
在“选区编辑”中可以上传另一张图片作为背景；服务端会按输出画布比例居中裁切并铺满，不会拉伸。编辑参数支持最多 60 步撤销与重做，选区笔刷也可逐笔撤销与重做。生成预览后，还会生成一组派生文件：
