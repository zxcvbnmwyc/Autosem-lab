...
AutoSEM 是一个本地优先的 AI 选区与局部编辑网站：用户上传图片后，只需用自然语言说出要处理的主体，也可以顺便描述效果。Qwen 会从任意自然表述中识别主体，并把其余意图改写为经过白名单校验的本地编辑计划；SAM2 负责生成像素级选区。只输入主体时流程停在可继续调整的选区，输入受支持的效果时再生成合成预览。手动分割 Agent、点／框提示和选区画笔仍完整保留。选区完成后，可补画、擦除、收缩／扩展、羽化，并导出透明抠图、纯色或虚化背景、主体与背景局部调色、描边、阴影及按主体裁切的高清 PNG。

它不是让大模型直接“画边界”：

```text
一句自然语言需求 ──> Qwen 提取主体并改写效果 ──> Qwen 视觉定位 ──> SAM2 ──┬──> 只有主体：可编辑选区
                                                                          └──> 包含效果：用户检查方案 ──> 本地全分辨率合成

手动模式：文字描述 ──可选──> Qwen 视觉定位 ──> Agent 决策 ──> 候选框 / 追问 / 手动提示
                                                          │
点 / 框 ─────────────────────────────────────────────────┼──> SAM2 Tiny ──> 质量复核 ──> 可编辑 Mask
                                                                                          │
选区笔刷 / 背景 / 主体调整 / 描边阴影 / 裁切 ───────────────────────────────────────────────┴──> 高清 PNG
```
...
手动点选和框选不需要任何外部模型。点击“一键处理”、“自动选区”或“查看推荐位置”时，当前图片和文字才会发送给 `.env` 配置的阿里云百炼 Qwen 视觉模型。一键处理采用“主体优先”：用户不必说“保留”、字段名或标准命令，Qwen 先从口语中提取被谈论或被操作的对象，再把其余描述语义改写为白名单内的编辑参数。例如“让奶酪更有质感”可映射为保守的对比度和锐化设置；只有“奶酪”则只生成选区。只有文字里完全没有主体或对象指代时，系统才会要求补充主体。

独立的 Qwen 定位阶段只给 SAM2 提供空间提示；Qwen 不能输出任意代码、路径、URL 或 mask 像素，也不能直接生成轮廓。SAM2 生成选区后，只有存在可见编辑效果时才进入方案检查和合成；没有额外效果时直接返回可编辑选区。定位出现多个候选或置信度不足时，会让用户选择对象，不会静默挑一个继续。当前一键能力覆盖单主体抠图、透明/纯色/模糊背景、边缘优化、主体与背景局部调色、主体柔化或锐化、透明度、描边、阴影，以及围绕主体的 free、1:1、4:5、16:9 裁切。删除后补图、添加或替换物体、扩图等生成式部分目前不会被伪装成已完成：若主体明确，系统会跳过无法执行的部分、显示说明，并继续生成该主体的选区或执行其余独立且受支持的操作；若没有主体，才会要求补充。启用内部锚点后，Qwen 在能可靠指出目标内部位置时会额外提供一个包含点，帮助 SAM2 区分框内相邻物体；无法可靠给点时会自动退回为仅使用候选框。
...
- `edited.png`：当前编辑结果；默认保持原图尺寸，启用按主体裁切时使用裁切后的尺寸，透明背景时为 RGBA PNG；
